...
export default {
  ...
  methods: {
    async getProfileByUserId(userId) {
      try {
        const response = await this.profileApiService.getProfileById(userId);
    ...
    async fetchClients() {
      this.loading = true;
      try {
        const response = await this.clientsService.getAllByProfileId(this.profileId);
    ...
    async onClientCreated(newClient) {
      try {
        await this.clientsService.create(newClient, this.profileId);
        await this.fetchClients();
        this.closeEditDialog();
      } catch (error) {
  ...
  <div class="client-management">
    <pv-card class="mb-4">
      <template #title>
        ...
        <p v-if="profileId">Profile ID: {{ profileId }}</p>
      </template>
      <template #content>
        <div class="card">
          <pv-toolbar class="mb-4">
            ...
            <template #end>
              <pv-button label="Add New Client" icon="pi pi-plus" @click="openNewClientDialog"
          ...
          >
            <pv-column field="personName" header="Name" :sortable="true" :filter-field="'personName'">
              <template #filter="{ filterModel }">
                <pv-input-text v-model="filterModel.value" type="text" class="p-column-filter"
            ...
            <pv-column headerStyle="width: 12rem">
              <template #body="{ data }">
                <pv-button icon="pi pi-eye" class="p-button-rounded p-button-info mr-2"
                           @click="viewClientDetails(data)"/>
                <pv-button icon="pi pi-pencil" class="p-button-rounded p-button-success mr-2"
